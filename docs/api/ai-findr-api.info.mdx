...
Para la **API privada de analíticas** documentada en esta sección bajo `/api/private/...`, utiliza la clave en el encabezado `Authorization` con formato `Bearer {api_key}` y acompáñala siempre con `X-Organization-Id`.
...
La referencia OpenAPI de esta página cubre principalmente los endpoints públicos y del widget. Si necesitas **leer métricas del dashboard, listar conversaciones o revisar el detalle de una conversación** desde un backend o dashboard propio, consulta la sección [Analytics](./analytics).
...
>
  ...
  >
    ...
    >
      ...
      label={"API Key: PrivateApiKeyAuth"}
      value={"PrivateApiKeyAuth"}
    >
      
      
      API key privada para endpoints de analíticas de lectura. Envíala en el header `Authorization` con formato `Bearer {api_key}` y acompáñala siempre con `X-Organization-Id`.
